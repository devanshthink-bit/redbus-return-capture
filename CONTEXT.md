...
home → outbound buses → outbound seat → outbound points
     → Return (calendar) → Your return (day list) → Review your trip → Pay → Ticket details
...
### The return question, and what it took to make it answerable (8 Sep)

`05`, `05a` and `05b` ask *When can you travel back?* with a **segmented control** — `Mode toggle`,
two `Segment / …` halves in a `#EDEDF2` track. **Neither half is selected until it is answered**,
because 65.2% of travellers were unsure and a default would default against them; until then
Continue sits at 40%.

I replaced this with redBus's own two-card-and-radio pattern (its *Change of plans?* sheet) and
**Devansh reversed it**: *"why did you change the toggle to this????? it was better."* The cards
were better evidenced and worse in place — on a screen that already carries a question, a calendar
...
This is that. All **28 frames** of *Screens · redBus return capture (iPhone 14)*, at 390 × 844,
walked in the order they are arranged on the canvas. It began at 23; 01a, 04a, 06b, 08c and 09a
came later.
...
is a **tab pair** — so 08b was rebuilt from 04 and 08c from 04a. `09a · Booking details` (9 Sep) is
the fifth: the sheet behind Pay's *View details*, built from `ReviewDetails.PNG`. It is a **sheet,
not a step** — in `OVERLAY` and `SKIP`, opened by the link and closed by its ×.
**An earlier, unrelated `09a` was deleted long before this one** — it was 09 with its content
...
`IMG_5214`, `IMG_5215`, `IMG_4991` and `IMG_5217`. **20 reads 8.75% over 4504pt**, which is the
same profile as 11's 10.35% over 4644pt and for the same reason: it is a clone of the ticket, so it
is mostly photographic and the drift accumulates. Read its bands for steps. 19a's 16.7% band at
the foot of the sheet is the primary button and its shadow.
