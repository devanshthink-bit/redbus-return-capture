...
| **Movable operators exist, none run on the chosen day** | Not knowable until the date is entered | The step appears, then *"No returns that day can be moved"* with **Show all returns** and **Choose another day**. Never a dead end |
...
| **v3** | `/v3.html` | *Calendar.* One tap for a fixed date, two for a range; books the **last movable** day and shows the saving. Carries the Idea 9 guard, **day-level movability**, an **auto-assigned return seat and boarding points** changed at Trip review, a visible change balance, and a **60-second undo** on the move |
